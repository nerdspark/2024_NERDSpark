...
public class AprilTagVisionIOLimelight implements AprilTagVisionIO {
    ...
    public AprilTagVisionIOLimelight(String identifier) {
        ...
        observationSubscriber = limelightTable
                .getStringTopic("json")
    ...
    public void updateInputs(AprilTagVisionIOInputs inputs) {
        ...
        for (TimestampedString timestampedString : queue) {
            ...
            LimelightHelpers.Results results = LimelightHelpers.parseJsonDump(timestampedString.value)
            ...
            double latencyMS =
                    results.latency_capture + results.latency_pipeline; // Calculates the total latency in milliseconds
            ...
            for (int i = 0; i < results.targets_Fiducials.length; i++) {
                tagIDs[i] = (int) results.targets_Fiducials[i].fiducialID; // Retrieves and stores the tag ID
                averageTagDistance += results.targets_Fiducials[i]
