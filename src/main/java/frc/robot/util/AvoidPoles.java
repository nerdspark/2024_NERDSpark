...
public class AvoidPoles {
    ...
    public static Translation2d adjustJoystick(
            Supplier<Pose2d> poseSupplier,
            Supplier<Translation2d> robotSpeeds,
            Supplier<Translation2d> joystickSpeeds,
            Supplier<Boolean> visionUpdated) {
        Translation2d pose =
                poseSupplier.get().getTranslation().plus(robotSpeeds.get().times(AvoidPolesConstants.lookAhead));
        ...
        for (int i = 0; i < AvoidPolesConstants.Poles.length; i++) {
            ...
            Rotation2d normalDirection =
                    poseToPole /*robotSpeeds.get()*/.getAngle(); // .plus(new Rotation2d(Math.PI/2.0));
            ...
            if (poseToPole.getNorm() < AvoidPolesConstants.distanceThreshold) {
                ...
                SmartDashboard.putString(
                        "poseToPole", String.format("(%.3f, %.3f)", poseToPole.getX(), poseToPole.getY()));
            }
        }
        if (robotSpeeds.get().getNorm() > AvoidPolesConstants.robotSpeedThreshold
                && visionUpdated.get()
                && Timer.getMatchTime() > AvoidPolesConstants.timeThreshold) {
            correction = correction
                    .times(joystickSpeeds.get().getNorm())
                    .times(
                            DriverStation.getAlliance().get().equals(Alliance.Red)
                                    ? AvoidPolesConstants.correctionGain
                                    : -AvoidPolesConstants.correctionGain);
        } else {
